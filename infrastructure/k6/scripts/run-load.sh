#!/usr/bin/env bash
# ==========================================================================
# LIBERTASIAN — k6 Load Test Runner
# Sustained load: ramp 0→20→50 VUs over 5 minutes
#
# Usage:
#   bash infrastructure/k6/scripts/run-load.sh
#   K6_BASE_URL=http://api:3001/api/v1 bash infrastructure/k6/scripts/run-load.sh
# ==========================================================================
...
echo "=========================================="
echo " LIBERTASIAN — k6 Load Test"
echo "=========================================="
echo "Profile:   load (0→20→50 VUs, 5min)"
...
docker compose -f "$COMPOSE_FILE" run --rm \
  ...
  k6 run /scripts/profiles/load.js

EXIT_CODE=$?

echo ""
if [ $EXIT_CODE -eq 0 ]; then
  echo "[PASS] Load test completed successfully — all SLO thresholds met."
else
  echo "[FAIL] Load test failed with exit code $EXIT_CODE — check threshold violations."
fi
