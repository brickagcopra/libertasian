export function paymentFailedTemplate(data: {
  ...
  const graceNoteParagraph = data.graceNote
    ? `
  <p>${escapeHtml(data.graceNote)}</p>`
    : '';
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p>We will automatically retry the payment on <strong>${escapeHtml(data.retryDate)}</strong>. To avoid service interruption, please update your payment method before then.</p>${graceNoteParagraph}
