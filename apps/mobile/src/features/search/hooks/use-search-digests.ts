 ...
 * full-text tab happened to return — so it could only surface digests attached
 * to those documents, and only if the document arm matched first. A digest whose
 * doctrine says "estafa" was unreachable unless its source decision also ranked
 * for "estafa".
 *
...
/**
 * The count badge on the Digests tab. Reads `meta.counts.digests` off the same
 * `/search` response the list renders — TanStack Query serves it from cache
 * under the identical query key, so this is one request, not two, and the badge
