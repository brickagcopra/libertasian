...
describe('parseReadAlong', () => {
  ...
  it('returns null for invalid JSON or a missing segments array', () => {
    ...
    expect(parseReadAlong('null')).toBeNull();
  });

  it('parses an empty manifest to zero segments', () => {
    const manifest = parseReadAlong(
      JSON.stringify({ version: 2, voiceId: 'Matthew', durationMs: 0, segments: [] }),
    );
