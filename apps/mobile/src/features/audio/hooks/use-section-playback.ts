...
import { audioRenditionQueryKey } from './use-audio-rendition';
...
/**
 * Drives per-section playback for the document reader. Ports
 * apps/web/src/features/audio/hooks/use-section-playback.ts.
 *
 * Deliberately NOT the digest continuous-playback hook: that one advances by
 * navigating to the next digest and extends its queue from a cursor-paginated
 * list endpoint. A document reader has neither — every section is already in
 * memory from `useDocumentSections`, and advancing is a state change inside one
 * screen, not a navigation. What IS carried over is the part that was hard to
 * get right (#244): the de-duplicated queue, the `nextId !== currentId` advance
 * guard, and re-reading the continue flag AFTER the await so a reader who
 * changes their mind mid-fetch is not yanked forward.
 *
 * `continueEnabled` is screen-local and defaults OFF, matching the digest
...
export function useSectionPlayback(
   ...
   *
   * A section may well be un-synthesized: the first not-ready GET is what
   * ENQUEUES it. Doing that here means the swap lands on a player that is
   * already loading rather than one that starts from nothing, and it gives the
   * guards below a real await to be checked across.
   *
   * The query key is `audioRenditionQueryKey(...)` — the SAME key the player's
   * `useAudioRendition` uses — so the warm-up is actually reused rather than
   * refetched. Note there is no `res.data` unwrap here (unlike web): the mobile
   * apiClient already strips the `{ success, data }` envelope, so unwrapping
   * again would cache `undefined`.
   */
  const prefetchRendition = useCallback(
    (sectionId: string) =>
      queryClient.fetchQuery({
        queryKey: audioRenditionQueryKey('legal_document_section', sectionId),
        queryFn: () =>
  ...
  const handleEnded = useCallback(() => {
    ...
    void (async () => {
      ...
      // document" off, or tapped a different section, while it was in flight —
