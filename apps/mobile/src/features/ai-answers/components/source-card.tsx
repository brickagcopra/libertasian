...
export function SourceCard({ source, index }: { source: AiAnswerSource; index: number }) {
  return (
    <View style={styles.sourceCard}>
      {/* The number is the whole point of the row: it is what the `[n]`
          markers in the answer above point at. Eight sections of one codal
          share a title, so without it the panel reads as eight duplicates. */}
...
const styles = StyleSheet.create({
  ...
  sourceTitleRow: { flexDirection: 'row', gap: 6 },
  sourceIndex: { fontSize: 13, fontWeight: '600', color: '#6b7280' },
  sourceTitle: { flex: 1, fontSize: 13, fontWeight: '600', color: '#1a56db' },
  sourceMeta: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 4, paddingLeft: 18 },
  sourceMetaText: { fontSize: 11, color: '#6b7280' },
  sourcePassage: { fontSize: 12, color: '#6b7280', marginTop: 6, paddingLeft: 18, lineHeight: 18 },
});
