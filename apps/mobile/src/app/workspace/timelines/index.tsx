...
import { TIMELINE_STATUS_LABELS } from '../../../features/timelines/types';
...
function TimelineCard({
  ...
  return (
    <TouchableOpacity
      style={styles.card}
      onPress={() => router.push(`/workspace/timelines/${item.id}`)}
      activeOpacity={0.7}
    >
        ...
        <View style={styles.badgeRow}>
          <View style={[styles.badge, { backgroundColor: statusColor.bg }]}>
            <Text style={[styles.badgeText, { color: statusColor.text }]}>
              {TIMELINE_STATUS_LABELS[item.status] ?? item.status}
            </Text>
          </View>
      ...
      {item.status === 'generating' && (
        <View style={styles.generatingRow}>
          ...
          <Text style={styles.generatingText}>Generating timeline...</Text>
        </View>
      )}
    </TouchableOpacity>
  );
...
export default function TimelinesListScreen() {
  const [statusFilter, setStatusFilter] = useState('');

  const { data, isLoading, isFetching, refetch } = useTimelines({
    limit: 30,
      ...
      Alert.alert(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Delete',
            style: 'destructive',
            onPress: () => deleteTimeline.mutate(id),
          },
        ],
      );
    },
  ...
  const renderItem = useCallback(
    ({ item }: { item: CaseTimelineListItem }) => (
      <TimelineCard item={item} onDelete={handleDelete} />
    ),
    [handleDelete],
  );

  const keyExtractor = useCallback(
    (item: CaseTimelineListItem) => item.id,
    [],
  );
        ...
        {isLoading ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>
              Generate chronological timelines from case histories and legal
              events
            </Text>
            <TouchableOpacity
