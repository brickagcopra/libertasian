...
function ComparisonCard({
  ...
  item: CaseComparisonListItem;
  onDelete: (id: string) => void;
}) {
  const typeLabel =
    COMPARISON_TYPE_LABELS[item.comparisonType] ?? item.comparisonType;
  ...
  return (
    <TouchableOpacity
      style={styles.card}
      onPress={() => router.push(`/workspace/comparisons/${item.id}`)}
      activeOpacity={0.7}
    >
      ...
      <Text style={styles.cardTitle} numberOfLines={2}>
        {typeLabel} — {item.documentIds.length} documents
      </Text>
      ...
      {item.status === 'generating' && (
        <View style={styles.generatingRow}>
          ...
          <Text style={styles.generatingText}>Comparing cases...</Text>
        </View>
      )}
    </TouchableOpacity>
  );
...
export default function ComparisonsListScreen() {
  const [statusFilter, setStatusFilter] = useState('');

  const { data, isLoading, isFetching, refetch } = useComparisons({
    limit: 30,
      ...
      Alert.alert(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Delete',
            style: 'destructive',
            onPress: () => deleteComparison.mutate(id),
          },
        ],
      );
    },
  ...
  const renderItem = useCallback(
    ({ item }: { item: CaseComparisonListItem }) => (
      <ComparisonCard item={item} onDelete={handleDelete} />
    ),
    [handleDelete],
  );

  const keyExtractor = useCallback(
    (item: CaseComparisonListItem) => item.id,
    [],
  );
      ...
      <View style={styles.container}>
        ...
        {isLoading ? (
          ...
          <View style={styles.emptyState}>
            <Ionicons name="git-compare-outline" size={48} color="#d1d5db" />
            <Text style={styles.emptyTitle}>No comparisons yet</Text>
            <Text style={styles.emptyText}>
              Compare multiple cases side-by-side to analyze doctrine, facts, or
              rulings
            </Text>
            <TouchableOpacity
