...
import { useGenerateContradiction } from '../../../features/contradictions/hooks/use-contradictions';
import type { ContradictionScope } from '../../../features/contradictions/types';
import { useSearch } from '../../../features/search/hooks/use-search';
import type { SearchResultItem } from '../../../features/search/types';

const SCOPE_OPTIONS: {
  value: ContradictionScope;
  label: string;
  description: string;
}[] = [
  {
    ...
    description: 'Analyze contradictions among the documents you select',
  },
  {
    value: 'topic_based',
    label: 'Topic-Based',
    description: 'Find contradictions across authorities on a specific topic',
  },
];
...
export default function CreateContradictionScreen() {
  const generateContradiction = useGenerateContradiction();
  const [scope, setScope] = useState<ContradictionScope>('selected');
  ...
  const canSubmit =
    !generateContradiction.isPending &&
    ((scope === 'selected' && selectedDocs.length >= 2) ||
      (scope === 'topic_based' && topic.trim().length > 0));
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await generateContradiction.mutateAsync({
        documentIds:
  ...
  return (
    <>
      <Stack.Screen
        options={{
          title: 'New Contradiction Analysis',
          headerRight: () => (
            <TouchableOpacity
            ...
            >
              ...
              >
                {generateContradiction.isPending ? 'Analyzing...' : 'Analyze'}
              </Text>
            </TouchableOpacity>
          ),
        }}
      ...
      >
        ...
        >
          ...
          <View style={styles.field}>
            <Text style={styles.label}>Analysis Scope</Text>
            {SCOPE_OPTIONS.map((opt) => (
              <TouchableOpacity
                key={opt.value}
                style={[
                  styles.typeOption,
                  scope === opt.value && styles.typeOptionActive,
                ]}
                onPress={() => setScope(opt.value)}
                disabled={generateContradiction.isPending}
              >
                <View style={styles.typeRadio}>
                  <View
                    style={[
                      styles.typeRadioInner,
                      scope === opt.value && styles.typeRadioInnerActive,
                    ]}
                  />
                </View>
                <View style={styles.typeContent}>
                  <Text
                    style={[
                      styles.typeLabel,
                      scope === opt.value && styles.typeLabelActive,
                    ]}
                  >
                    {opt.label}
                  </Text>
          ...
          {scope === 'topic_based' && (
            <View style={styles.field}>
              ...
              <TextInput
                ...
                placeholder="e.g., Constructive dismissal standards"
                placeholderTextColor="#9ca3af"
                editable={!generateContradiction.isPending}
              />
            </View>
          )}
          ...
          <View style={styles.field}>
            <Text style={styles.label}>
              {scope === 'selected'
                ? `Select Documents (${selectedDocs.length}/10) *`
          ...
          <View style={styles.infoCard}>
            <Text style={styles.infoText}>
              The AI will analyze selected authorities for contradictions,
              inconsistencies, and conflicting doctrines. Each contradiction is
              rated by severity. Generation may take up to 90 seconds.
            </Text>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
