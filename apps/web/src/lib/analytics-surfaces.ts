/**
 * Navigation → analytics surface mapping (web).
 *
 * Canonical copy. `apps/mobile/src/lib/analytics-surfaces.ts` mirrors the
 * locked region below verbatim; neither app can import from the other, so the
 * two are pinned together by a byte-for-byte test on each side — the same
 * arrangement as `features/admin/budget-scopes.ts`. A drifted copy would put
 * the same screen under two different surface names and split its row on the
 * dashboard.
 */
