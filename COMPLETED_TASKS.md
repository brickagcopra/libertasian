...
8. [x] **`bulk-pregenerate-audio.ts` deliberately untouched** — it is Polly-shaped (`MAX_BILLED_CHARS` skip-long, `--char-budget` metering, cost = 2 x len) and none of that applies to Kokoro. The reconciler is the backfill mechanism.

### Acceptance evidence

9. [x] **The GPU image builds and serves**: `torch 2.13.0+cu129 cuda 12.9`, weights baked (`preloaded hexgrad/Kokoro-82M af_heart`), `/health` 200 in 7 s, `/synthesize` 401 with no token / 401 with a wrong token / **200 with 29,088 B of mp3 and 16 mark lines** with the right one, 200 when no token is configured, and `Internal Server Error` with no traceback on a 500.
10. [x] **4,004 api unit tests pass** (32 new) and **25 tts-service tests pass** (13 new); `tsc --noEmit`, `ruff`, and `mypy --strict` clean apart from the two untyped-import errors already on `main`. No test touches host CPU or disk — the Python 200-path tests monkeypatch `synthesize_document` and every budget assertion is arithmetic under fake timers.
11. [ ] **The GPU path itself is UNVERIFIED and must not be reported otherwise.** The build host has no NVIDIA device, so every check above ran the GPU image with `TTS_DEVICE=cpu`. Throughput, VRAM per worker, and the right `KOKORO_REALTIME_FACTOR` for that box are all still assumptions. See PENDING_TASKS.md.
