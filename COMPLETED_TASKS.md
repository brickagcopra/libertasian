...
## 2026-09-01 — sign-in responses were missing the org fields the purchase flow needs

Branch `fix/auth-response-org-fields`, **PR #454**.

**The symptom was on mobile; the bug was one object literal on the server.** `usePurchaseOptions` (`apps/mobile/src/features/purchase/hooks/use-purchase-options.ts:75`) returns early when `organizationId` is falsy, so `configurePurchases()` never ran and the RevenueCat SDK was never initialized for the session — "Plans are not available right now" on the purchase screen, and Restore Purchases throwing into an unconfigured SDK. Prod confirms it: RevenueCat has no session for the org from any build-29 device session.

**Why `organizationId` was undefined.** `UsersService.sanitize()` returns no org fields. `GET /users/me` adds them back from the JWT payload (`users.controller.ts:57-63`). The five auth sites in `auth.service.ts` — register, the login MFA challenge, login success, Google, Apple — did not; each returned `{ ...sanitize(user), isPlatformAdmin }`. Whichever call populated the client's auth context first decided whether the field existed.

**The fix is server-only, deliberately.** The client is correct as written; changing it would need a new iOS build, and the point of this fix is that it does not. A single private `AuthService.buildAuthUser(user, membership, isPlatformAdmin)` builds the shape once, matching `/users/me` field for field, and is applied at all five sites. `isPlatformAdmin` is unchanged at every site, including the `false` literal on the MFA challenge. Two details worth remembering:

- `login()` now resolves the membership **before** the MFA branch so the challenge response carries the org fields too. The `!membership` rejection stays where it was, so an MFA user with no active membership still gets the challenge before the 401 — behaviour unchanged, one extra query on the challenge path only.
- No membership yields **explicit `null`, not a missing key**, matching what `apps/mobile/src/features/auth/types.ts` already declares.

**Both new tests were verified to fail on the pre-fix code**, not merely to pass on the new one. `auth-user-contract.spec.ts` builds both shapes from the real `sanitize` and fails if either side grows or drops a field the other lacks — the drift guard these two paths never had. `auth-org-fields.e2e-spec.ts` pins the wire bodies for `/auth/login`, `/auth/register`, `/auth/google/mobile` and `/auth/apple/mobile` against the active membership, plus a superset check against `/users/me`. `createTestApp()` gained an optional module-builder hook so a suite can stub `SocialTokenService` (the real one verifies against live Google/Apple JWKS).

**Gates.** `pnpm --filter api test` 4703/4703 (210 suites); `tsc --noEmit` exit 0; auth-surface e2e 131/133 across 8 suites. The two e2e failures — `auth.e2e-spec.ts › two concurrent refreshes with the same cookie` and `organizations.e2e-spec.ts › should invite a new member` — **reproduce identically on `main`** against the same database, checked by checking `main` out and re-running them.

**Environment note for whoever runs e2e next.** The local dev DB is behind its migration history (`users.apple_id` does not exist; every request 500s) — the drift is 9 unapplied migrations plus one applied migration absent locally. This ran against a throwaway `libertasian_e2e` database built with `prisma migrate deploy` rather than resetting the dev DB. The full e2e suite still cannot complete locally: several suites hang because the Python services (rag, ocr) are not up.
...
### Amended in review (2026-08-05) — the standard is adaptive, and the report now says so

The breadth denominator was validated independently on prod after #356 deployed, over 64 questions (8 per subject, all 8 subjects): **denominator 3 in 42 of 64 (66%), 2 in 20 (31%), 1 in 2 (3%), 0 in none** — breadth is never undefined. Distinct documents per retrieved set: min 1, median 3, max 8. Citable passages: min 5, median 6, of 8. The `section_id` gap does not inflate it — 42 questions reach denominator 3 counting all documents against 40 counting only citable ones.

13. [x] **The bar asks different things of different questions, and that is now recorded rather than glossed.** At denominator 3 (66%) 0.70 means two distinct clean authorities; at denominator 2 (31%) **one clean citation scores 0.75 and passes**; at denominator 1 (3%) one citation scores 1.0. Defensible — an answer cannot cite what retrieval never offered, and a fixed denominator would fail well-grounded answers for the corpus's gaps — but the consequence is that **any aggregate pass rate partly reflects retrieval breadth, not answer quality alone**. Two identical answers can land on opposite sides of the bar.
14. [x] **The dry run breaks the distribution out BY DENOMINATOR (1 / 2 / 3 / unknown), by subject, and as a subject × denominator cross-tab.** A single blended rate would hide the effect, and retrieval breadth varies systematically by subject (legal_ethics 2.9 distinct documents, criminal_law 5.0), so a per-subject rate carries that subject's retrieval profile inside it.
15. [x] **The counts behind each score are now persisted**, in a `grounding` block on `structured_answer_json`: emitted / valid / fabricated ids, cited and available documents, breadth denominator. Without it the denominator is unrecoverable — the retrieved set is not stored — and reconstructing it from surviving citations yields a **lower bound** that would misclassify rows downward and hide exactly what the breakout exists to show. **Counts only, no BM25 scores, no document ids, no passage text**, because that block is served verbatim to the public endpoint; a test asserts every value is an integer and that no document id or score appears in it. Pre-#357 rows have no block and report `denominator=?` rather than being guessed into a bucket.
16. [x] **Weights untouched.** Nothing in the validation is a reason to move 0.70 — per CLAUDE.md the answer to a disappointing distribution is to fix what the terms measure.
...
**Session note.** This session was interrupted by a power cut and resumed from the working tree on 2026-08-04. State at resume: all source edits present and unstaged, `tests/test_opensearch_client.py` untracked, nothing committed.

**The finding.** `services/rag-service/src/shared/opensearch.py` built its `httpx.AsyncClient` with a base URL and a timeout and **nothing else** — no `auth`, no `verify`. `docker-compose.prod.yml` points it at `https://opensearch:9200`, and that container runs with `OPENSEARCH_INITIAL_ADMIN_PASSWORD` set and serves a self-signed certificate, so every request from the RAG service could only fail the TLS handshake. There is no code path in that file that could have supplied credentials — `Settings` had **no** `opensearch_username` / `opensearch_password` fields to supply.

**Why it was invisible.** `opensearch_search` caught `httpx.HTTPError`, logged at WARNING, and returned `{"hits": {"total": {"value": 0}, "hits": []}}`. A total connectivity outage was therefore byte-identical to a query that legitimately matched nothing, and every surface downstream — answers, memos, flashcards, pleadings, hearing prep, research workspaces — degraded to confident priors-only output or an abstention rather than an error.

### What changed

1. [x] **`Settings` gains `opensearch_username`, `opensearch_password`, `opensearch_verify_ssl` — and the credentials read the UNPREFIXED env pair.** `env_prefix` is `RAG_`, so declaring the fields alone would have read only `RAG_OPENSEARCH_USERNAME`, which nothing sets; the service would have traded `CERTIFICATE_VERIFY_FAILED` for a 401. `validation_alias=AliasChoices("RAG_OPENSEARCH_USERNAME", "OPENSEARCH_USERNAME")` (prefixed first, so an explicit per-service override still wins) picks up the `OPENSEARCH_USERNAME`/`OPENSEARCH_PASSWORD` the rag-service container **already** carries via `env_file: .env`. **The fix is therefore code-only: no `.env` edit, no compose edit, no new secret.**
2. [x] **Basic auth is attached only when BOTH username and password are set**, so an unsecured local cluster still works and a half-configured pair cannot produce a broken `Authorization` header. Asserted in all four combinations.
3. [x] **`verify` follows `opensearch_verify_ssl`, default `false`** — the internal cert is self-signed and the hop never leaves the container network. Flip it once a real cert exists.
4. [x] **`opensearch_search` no longer fakes an empty result set.** It logs at ERROR and re-raises. The docstring states the contract: a caller that genuinely wants to degrade must catch it and say so at the call site.
5. [x] **Both call sites now make that choice explicitly.** `hybrid_retrieve`'s **BM25 arm propagates** (no BM25 means no results, and the caller must be told); its **kNN arm degrades to BM25-only** with an ERROR log, because a vector-index failure leaves a complete keyword result set standing and `total_knn_hits: 0` records what happened. `_search_codal_candidates` keeps its soft failure — codal candidates only enrich a prompt — but now says so in a comment instead of inheriting silence.
6. [x] **Startup ping.** `ping_opensearch()` runs in a FastAPI `lifespan` and logs the cluster distribution/version plus `verify_ssl` and whether auth is configured; on failure it logs `OPENSEARCH UNREACHABLE … ALL retrieval will fail` naming the four env vars. **Deliberately non-fatal** — priors-only generation still runs — but never again silent. The lifespan also closes the client on shutdown, which nothing did before.
7. [x] **The codal-suggestion search was querying an index that has never existed.** `_search_codal_candidates` searched `legal_documents` and read a `content` field; the API's real alias is `legal_documents_keyword` and the mapping's body field is `plain_text` (`apps/api/src/modules/search/index-mappings.ts:30,136`). Every codal candidate lookup would have 404'd even on a healthy cluster — and, under the old swallow-everything client, returned `[]`.

### Acceptance evidence

8. [x] **`tests/test_opensearch_client.py` — 18 new tests**: auth wired/absent across all four credential combinations, `verify` in both states, base URL, a real `httpx.BasicAuth` header end-to-end, client reuse and rebuild-after-close; connect/404/401 failures all propagate; a genuine zero-hit response still returns normally (the distinction the whole change is about); ERROR-level logging; ping success and failure.
9. [x] **5 new tests in `tests/test_retrieval.py`**: BM25 failure propagates out of `hybrid_retrieve`; a failing kNN arm degrades to BM25-only and logs at ERROR; `retrieve_by_query` and `retrieve_by_document_id` propagate.
10. [x] **rag-service suite: 615 passed.** The 40 errors in `tests/test_routers.py` are **pre-existing and unrelated** — its `client` fixture is a plain `@pytest.fixture` around an async generator, which pytest-asyncio's strict mode refuses. Verified by stashing this branch and re-running: identical 40 errors on a clean tree. **`ci.yml` does not run the Python services at all**, so no CI signal exists for any of this either way.
11. [x] **Confirmed on prod 2026-08-05 (brick), which upgraded this from inference to measurement.** A traceback captured inside the prod rag container: `httpx.ConnectError [SSL: CERTIFICATE_VERIFY_FAILED]`, swallowed into `{"hits":{"total":{"value":0},"hits":[]}}`. Both arms were then tested separately against the live cluster: **`verify=False` alone → 401 Unauthorized**; **`verify=False` + basic auth → 200 with 10k+ hits**. Neither arm is sufficient on its own — which is why the credential-source tests above are load-bearing rather than decorative.
12. [x] **No new gateway plumbing needed for the 500s.** `apps/api/src/modules/ai-answers/ai-answers.service.ts:72-73` already throws on a RAG non-2xx; that path was simply unreachable while the client manufactured empty result sets. Surfacing retrieval errors makes it reachable, it does not require it to be built.
13. [ ] **Still unmeasured: how long this was broken, and what it produced.** Every query answered during the outage was answered with no sources. `docker compose logs rag | grep -c "OpenSearch search failed"` over retained logs is the closest available signal — see PENDING_TASKS.md.
...
## 2026-07-29 — #336: a flat timeout made average-length digests unsynthesizable

**The finding.** Prod measured Kokoro on-box at **41.5 items/hour** (~156 s audio/item, 8 of 12 cores). `KokoroClient.TIMEOUT_MS` was a flat `300_000` for every request regardless of length, and `MAX_ATTEMPTS = 3` retried every failure identically. Digest `0a8d731f-8b21-4332-b001-93779ebdf054` — **2,238 chars, near the 2,032-char corpus average** — produces ~166 s of audio, which at the measured 1.0–1.4x realtime is ~184–232 s of wall clock; queueing pushed it past 300 s. It failed permanently after **3 attempts = 15 min of 8-core CPU for nothing.** Synthesis is compute-bound and deterministic: a request that ran out of budget runs out of the same budget again.

### What shipped

1. [x] **Length-proportional budget.** `max(60s, chars / 13.5 chars-per-audio-second x KOKORO_REALTIME_FACTOR)`, capped by `KOKORO_TIMEOUT_CEILING_MS`. 13.5 is the measured af_heart rate; the 2.5 default factor is the measured CPU worst case with headroom. The 2,238-char digest gets 414,445 ms instead of 300,000; the 4,877-char one gets 903,149.
2. [x] **Failures are classified, not blanket-retried.** `transient` (network / 5xx / 408 / 425 / 429) keeps 3 attempts with backoff; `timeout` is retried **once** and only with a 1.5x budget (never at all if the ceiling means the retry cannot be larger); `permanent` (401, other 4xx, malformed payload) fails immediately. The processor throws BullMQ `UnrecoverableError` for the non-transient classes, so the job is marked failed without consuming remaining attempts.
3. [x] **Text no allowed budget can cover is refused BEFORE the call** (`text_too_long`) rather than started and abandoned at the ceiling.
4. [x] **`audio_renditions.failure_reason`** (nullable `VARCHAR(200)`, additive migration) records which class and why. Written once per job on the final attempt — a mid-retry `failed` row would contradict a later success — and **never over a `ready` row**, so a forced regen that fails cannot pull serviceable audio out of circulation. Cleared to NULL when the rendition later succeeds; never carries document text.
5. [x] **GPU image as a SEPARATE file** (`infrastructure/docker/Dockerfile.tts.gpu`). `Dockerfile.tts` still produces today's CPU-only image, because prod has no GPU. CUDA base + **cu129** torch wheels (2.13.0 is published for cu126/cu129/cu130 but **not** cu128 — verified against the index before pinning), one lockfile serving both images via a single-package reinstall from the CUDA index, and a build-time assertion that `torch.version.cuda` is populated so an unswapped CPU wheel fails the BUILD.
6. [x] **`TTS_DEVICE`.** `auto` (prod) passes no `device` to `KPipeline` — behaviour-identical to before the knob existed. The GPU image sets `cuda` explicitly, and kokoro then raises `RuntimeError: CUDA requested but not available` on a box with no device. A silent CPU fallback on a rented GPU looks like a working service while running at ~1x realtime; that was the failure worth designing against.
7. [x] **Bearer auth on `/synthesize`** (`secrets.compare_digest`, never echoes the presented token). `/health` stays open because the container HEALTHCHECK curls it with no credentials. Both sides no-op when `TTS_AUTH_TOKEN` is unset, so prod's in-network call is unchanged after deploy and enabling it is a per-deployment decision.
8. [x] **The device is reported, not inferred.** `KPipeline` was constructed with no `device`, so kokoro's own `cuda if torch.cuda.is_available() else cpu` decided it silently. `src/device.py` now resolves it explicitly (unset/`auto` → the same answer, but stated), logs it once at startup, and **`/health` returns `device`, `cuda_available`, `device_name`, `workers`, `threads_per_worker`.** That distinguishes three states previously indistinguishable from outside: on GPU, on CPU because no device is visible, and configured for CUDA in a container that never got one (`device=cuda` + `cuda_available=false` — synthesis will fail).
9. [x] **Worker/thread shape is device-aware.** The old `2 x 4` was CPU-shaped by construction and the config comment said so. Unset now resolves to 2 x 4 on CPU and **1 x 8 on CUDA**, because uvicorn workers do not share the model — a second worker on one card is a second full copy of the weights (~2.9 GiB measured) for no extra throughput. `src/serve.py` resolves the device BEFORE launching uvicorn, which a shell `--workers ${TTS_WORKERS:-N}` cannot; the CPU image keeps its shell CMD unchanged. An explicit override always wins, and `TTS_WORKERS>1` on CUDA is honoured with a warning rather than clamped, since a multi-GPU host is a real case.
10. [x] **`bulk-pregenerate-audio.ts` deliberately untouched** — it is Polly-shaped (`MAX_BILLED_CHARS` skip-long, `--char-budget` metering, cost = 2 x len) and none of that applies to Kokoro. The reconciler is the backfill mechanism.

### Acceptance evidence

11. [x] **The GPU image builds and serves**: `torch 2.13.0+cu129 cuda 12.9`, weights baked (`preloaded hexgrad/Kokoro-82M af_heart`), `/health` 200 in 7 s, `/synthesize` 401 with no token / 401 with a wrong token / **200 with 29,088 B of mp3 and 16 mark lines** with the right one, 200 when no token is configured, and `Internal Server Error` with no traceback on a 500.
12. [x] **`/health` proves the device on the built image.** With the image default (`TTS_DEVICE=cuda`) and no `--gpus`, the body reads `"device":"cuda","cuda_available":false,"device_name":null,"workers":1,"threads_per_worker":8` and `/synthesize` returns 500 with `CUDA requested but not available` — the misconfigured-container state is now readable in one request instead of inferred from throughput. Forced to `cpu` the same image reports `"device":"cpu"..."workers":2,"threads_per_worker":4`, i.e. `src/serve.py` sized uvicorn to the resolved device (1 worker on cuda, 2 on cpu).
13. [x] **4,004 api unit tests pass** (32 new) and **39 tts-service tests pass** (27 new); `tsc --noEmit`, `ruff`, and `mypy --strict` clean apart from the two untyped-import errors already on `main`. No test touches host CPU or disk — `torch.cuda.is_available` is monkeypatched in every device test (a test whose outcome depended on whether the runner has a GPU would assert nothing), the Python 200-path tests monkeypatch `synthesize_document`, and every budget assertion is arithmetic under fake timers.
14. [ ] **The GPU path itself is UNVERIFIED and must not be reported otherwise.** The build host has no NVIDIA device, so every passing check above ran with `TTS_DEVICE=cpu`; the `cuda` runs are the *failure* rows. Throughput, VRAM per worker, the `device_name` field against a real card, and the right `KOKORO_REALTIME_FACTOR` for that box are all still assumptions. See PENDING_TASKS.md.
...
## Session 205 — Search Phase C1: derivative content extraction + index mapping (2026-07-26)
...
3. [x] **MCQ answer-key rule enforced at extraction, not at query time.** Only the stem and option *text* are extracted; `isCorrect`, `rationale` and `explanation` are never emitted and the mapping has **no field to hold them**. Anything that reaches the index leaks via highlight fragments, `_source`, `fields` and aggregations, so excluding it per-query would require every present and future call site to remember. Under `dynamic: 'strict'` the absent mapping is a live control: a regressed extractor fails the write with `strict_dynamic_mapping_exception` instead of silently indexing a rationale. Deliberately **no generic fallback walker** for unknown types, so a future MCQ variant that renames the field cannot auto-index it. An admin-facing rationale search, if ever built, is a separate index.
4. [x] **Derivatives mapping** to existing conventions: `dynamic: 'strict'`, versioned physical name behind an alias, `keyword` for filterable metadata, `text` for the body. **BM25 only — no `knn_vector`, deliberately:** embedding ~100k short, heavily-titled artifacts costs more than the recall it buys, and they are already reachable semantically through their source document. Adding vectors later is a new physical index + alias flip, so nothing is one-way.
5. [x] `INDEX_VERSION` **not** bumped, the three existing mappings untouched, and `DERIVATIVES_INDEX_ENTRY` deliberately left **out** of `INDEX_TOPOLOGY` (pinned by a test at exactly 3 entries) so the rebuild job behaved exactly as before and adding it had to be a deliberate act in C2. `organization_id` / `visibility` mapped but not yet filtered on — they must exist before C2 can filter, since `dynamic: 'strict'` rejects an unmapped field.
6. [x] **The 11 content shapes moved to `@libertasian/types`.** `apps/api` provably cannot import `apps/web/src`: `apps/api/tsconfig.json` pins `rootDir` to `./src`, so a cross-app import fails `tsc` with TS6059 and breaks the lint gate (verified, not assumed). Loosening `rootDir` would risk the `dist/` layout `CMD ["node","dist/main"]` depends on — the same class of change that broke prod in #309. Copying the shapes into the API would let extractor and renderers drift. The web fixtures module re-exports every constant, so all 12 renderer tests kept their original import path.
7. [x] **`apps/web/vitest.config.ts` now aliases `@libertasian/types` to source.** That re-export was the first runtime (non-`import type`) import of the package in web, surfacing the same latent gap #309 fixed for the API container: `main` points at a gitignored `dist/`, so under a bare `vitest run` the constants resolved to `undefined` and 21 renderer tests failed. Twin of the `moduleNameMapper` the API jest config already had. Confirmed against a stashed baseline: web derivatives 15/15 files, 65/65 tests before and after.

---

## Session 206 — Search Phase C2: derivative indexing + visibility filter (2026-07-26)

**Merged:** #311 → main (`d4077df`), `feature/PHASE-C2-derivative-index-ingest`. Branch deleted. `INDEX_VERSION` unchanged — the derivatives index carries its own version through its entry, so the three verified prod indices were not forced into a needless rebuild.

1. [x] **Rebuild-job derivatives phase** mirroring the document phase: keyset pagination by `id` (never OFFSET); `where: { deletedAt: null }` on the count *and* every page, so soft-deleted rows are excluded from source and destination alike (otherwise the source floor and the index disagree by exactly the soft-deleted set); `_bulk` in batches of 500; `refresh_interval` relaxed to `30s` on the destination and restored to `5s` in a `finally`, so a failed run cannot leave a 30s-stale index behind an alias. Body text comes from the C1 extractor — shapes are not re-derived, which is what keeps the MCQ rule in exactly one place. Runs **last**, so a derivative failure cannot delay or destabilise the keyword index.
2. [x] **Verification reuses #308's measured model:** `sourceCount` from PostgreSQL, `destCount` from `_count` after an explicit refresh, status `verified` / `source_missing` / `mismatch` / `failed`. What `_bulk` claimed it accepted is recorded as `reportedCreated` for diagnosis and decides nothing. An unverified index does **not** get its alias swapped — it is left for inspection, the alias keeps serving its previous target, and the name appears in `aliasesSkipped`.
3. [x] **Per-item bulk failures THROW** — `bulkIndexDerivatives` is deliberately not a variant of `bulkIndexDocuments` (which counts per-item errors and returns them). The derivatives index is `dynamic: 'strict'` with no field for an MCQ answer key, so the way a regressed extractor surfaces is *precisely* a per-item `strict_dynamic_mapping_exception`. Counting that as "1 error out of 500" would demote the security control to a log line and let the alias swap proceed.
4. [x] **`organization_id` is omitted, not blanked.** For a NULL column the key is **absent** — never `''`, never a sentinel. `toDerivativePayload` spreads it conditionally and `bulkIndexDerivatives` strips it again as defence in depth. This is load-bearing: the public branch is `must_not exists organization_id`, and `exists` is false only for an *absent* field, so a sentinel would flip every null-org row out of the public branch and make those rows invisible to everyone. Tests demonstrate exactly that failure.
5. [x] **`buildDerivativeVisibilityFilter`** — two explicit branches, no implicit fallthrough. **public** = `visibility = public_editorial` AND `must_not exists organization_id` (the second half is not redundant: an org-owned row marked `public_editorial` is still that org's row, and without it that row is world-readable). **authenticated** = the public branch OR (`organization_id` = the caller's own org AND visibility on an enumerated allowlist). The allowlist is a `terms` set, so an unrecognised visibility value matches no branch and is **excluded**, not passed through. A null principal — or one without a usable org claim — gets the public branch, written out explicitly rather than falling out of a missing `else`. `organizationId` is documented as JWT-claims-only; a client-supplied value would turn this into an org-enumeration primitive.
6. [x] **Authorization tests evaluate the generated DSL against documents** rather than asserting its shape. A shape assertion ("contains a `must_not exists` clause") passes just as happily when the clause sits in the wrong branch, or when a second branch quietly widens what the first restricted. The property that matters is a statement about matching, so it is tested by matching. The matcher implements OpenSearch `exists` semantics (true for a present field *including* an empty string) and is itself tested first, since every other assertion depends on it.
7. [x] Covered: null-org rows match only the public branch; org A rows never reach a caller in org B (checked from both sides); an unauthenticated caller receives no org-scoped row; unknown visibility values excluded for public and owning-org callers alike; sentinel org ids shown to break the public branch; `INDEX_TOPOLOGY` has exactly 4 entries. Plus rebuild coverage of keyset pagination, soft-delete exclusion, refresh-interval restore on throw, each measured status, and that a derivative failure still leaves the keyword alias swapped.
8. [x] Gates: `pnpm --filter api test` 181 suites / **3,816** tests / 12 snapshots; `pnpm lint` 5/5. Zero `any`.
9. [x] **Prod-run note recorded in the PR:** `allocateTargetIndex` is blue/green, so with `*_v3` already existing in prod the run builds `legal_documents_keyword_v3_r1`, `legal_documents_vector_v3_r1` and `user_uploads_searchable_v3_r1` while the derivatives index is fresh at `derivative_artifacts_v3`. Existing behaviour, not something C2 changed — the `_r1` names are expected, not a symptom.

---

## Session 207 — Search Phase C3: federated derivative search surface (2026-07-26)
...
3. [x] **The derivative arm is BM25 only, and must stay that way** — the index carries no `knn_vector` field (C1's deliberate cost decision), so a kNN clause would match nothing. A test asserts the serialized query body contains no `knn` / `embedding` / `script_score`.
4. [x] **Authorization: the visibility filter is a REQUIRED argument** of both `buildDerivativeQueryBody` and `searchDerivatives` — typed non-optional, because a derivative query without it is a cross-tenant read, and that should be impossible to build by forgetting an argument rather than merely discouraged in a comment. It goes in `filter`, not `must`, so it is a hard boolean gate contributing nothing to the score. The principal comes from verified JWT claims in the controller and `dto` is never consulted for identity; a test sends a body carrying `organizationId: 'org-victim'` and asserts it changes nothing.
5. [x] **Derivative results are deliberately NOT cached.** Their visibility depends on the caller's organization, so a correct key must include the org — and a key that is wrong by omission serves one tenant's private artifacts to another. The document arm keeps its existing cache entry, and `scope` is excluded from that key because the document results for a query are identical either way.
6. [x] Results carry `kind: 'document' | 'derivative'` only on a federated request, where `meta` also gains scope / counts / warnings. Ordering within each kind is BM25; **the two lists are concatenated, not globally ranked**, because BM25 scores from two indices with different mappings and term statistics are not comparable — interleaving them by score would be inventing a ranking. Cross-corpus fusion needs a reranker over the merged set: that is C4.
7. [x] **Highlighting restricted to `title` and `body_text`, named explicitly**, so a future mapping addition cannot start emitting fragments from a field nobody reviewed for disclosure. Derivative sources also pass through `sanitizeDerivativeSource`, which strips the MCQ answer-key fields — a third layer behind the extractor and the strict mapping, for documents those paths do not cover (a manual write, a restored snapshot). A test pins that it is a no-op for a document the rebuild job actually writes, so it cannot mask an extraction regression.
8. [x] **Degradation:** a derivative arm that throws or times out yields a warning plus the document results, never a failed request — document search is the primary surface. The 5s timeout is on the query body.
9. [x] **Fixed a `describeTopology` false mismatch.** `expectedPhysical` is the base `*_v3` name, but `allocateTargetIndex` is blue/green, so a re-run at an unchanged `INDEX_VERSION` lands on `*_v3_r1` — and the endpoint reported all four aliases as mismatched immediately after a correct, verified rebuild, which is a false alarm exactly when an operator most needs to trust it. `matchesExpected` now treats a `<base>_r<N>` suffix as a match while rejecting a different version and lookalike suffixes (`_rX`, `_r1x`, `_r`). Raw `expectedPhysical` and `currentTargets` are still reported, so the boolean is never the only thing visible.
10. [x] The DSL matcher moved to `apps/api/src/testing/opensearch-dsl-matcher.ts` so C2's visibility spec and C3's federated spec share one implementation of `exists` semantics; unsupported clause types throw rather than defaulting, since a silent default in a security test is a false pass.
11. [x] Tests: `scope` omitted === the legacy response (deep equality); a private null-org row absent for every caller including admins; org A rows absent for a caller in org B; a public_editorial null-org row present for an anonymous caller; `scope='all'` returns both kinds and `scope='derivatives'` never queries documents; an `mcq_question` result exposes no `rationale` / `isCorrect` / `explanation`; base, `_r1` and genuinely-wrong topology targets.
...
10. [x] **CSP: the real R2 origin in BOTH `media-src` and `connect-src`.** The player streams the mp3 through `<audio>` (media-src) **and** `fetch()`es the marks manifest (connect-src). Signed R2 URLs are cross-origin, so missing either directive makes the browser block the request with **no server-side error** — the failure mode is a silently dead player. **Amended after review:** the first pass shipped a literal placeholder token to be substituted on prod. That was wrong — `nginx.conf` is a tracked file bind-mounted from the repo, so a prod edit breaks the no-edits-on-prod rule and is clobbered by the next `git pull`. The account id is not a secret (it is the host of every presigned URL the browser already receives), so it is committed literally. Verified only `media-src` and `connect-src` differ from the original; the other 8 directives are byte-identical, and line 90 is the only CSP in the file.
...
12. [x] **The reconciler's disk guard now knows storage went remote** — amended after review, since the change that causes the problem should ship with the fix. It `statfs`es a local path, so with renditions streamed to R2 it would block on a number describing nothing and an unrelated local disk issue would halt the backfill. `AudioStorageService.isRemote` is the single source of truth (no second env read), and when remote the guard is skipped with one `audio_reconcile_disk_guard_skipped` log per tick. **The tier-3 "unmeasurable" refusal also had to be conditioned on it** — otherwise remote mode would read as unmeasurable and refuse tier 3 forever, the exact opposite of what moving storage off-box achieves. Local mode untouched; both modes tested, including that remote storage removes the disk constraint and **not** the two-flag requirement.
