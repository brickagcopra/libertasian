...
**A leg that never contributes no longer looks healthy.** `SearchResult` gains `degraded` / `degraded_legs`. `hybrid_retrieve` now logs at ERROR with the leg name and, critically, the **OpenSearch reason pulled out of the response body** (`error.root_cause[].reason`) — without it, httpx reports only "Client error '400 Bad Request'", which reads identically for a wrong field name, a malformed filter and a missing knn plugin. A missing embedding is reported the same way (`knn:not_configured`), because a leg that is never invoked is exactly as invisible as one that fails every time. `total_knn_hits: 0` was the only prior trace and it is indistinguishable from a healthy leg with no vector matches.
