...
- [ ] **Substitute `R2_ENDPOINT_ORIGIN` in `infrastructure/nginx/nginx.conf` (prod op, brick's call).** It is a literal placeholder in **both** `media-src` and `connect-src`. Cross-origin audio blocked by CSP produces **no server-side error** — the player just silently fails, so this is the item most likely to be missed. Create the R2 bucket **private**; presigned GETs only, never a public `r2.dev` domain.
- [ ] **The reconciler's disk guard stops meaning anything once audio is off-box.** It `statfs`es a local path. Tier 3 was gated on ~158 GB of decisions vs ~142 GB free locally — the exact constraint R2 removes — so the guard needs to become a bucket quota/billing check before decisions are enabled. Flagged in code, behaviour unchanged.
- [ ] **Size tier 3 against ~1x realtime before enabling decisions.** At the measured throughput the decision backfill is on the order of 8,000 worker-hours of synthesis. That is a capacity decision independent of storage, and the old 3.59x constant would have understated it ~3.7x.
