...
- [ ] **Watch for `knn:not_configured` after deploy — it should appear on every query today, and disappear the moment the embedding client lands.** The signal is `SearchResult.degraded_legs`, not the logs: it is warned about **once per process at WARNING**, deliberately, because an ERROR per request would flood Sentry for as long as the gap exists. The `Hybrid retrieval DEGRADED` ERROR line is reserved for a leg that actually failed at runtime. If the flag does *not* appear on responses, something else is wrong.
...
- [ ] **Bring the remaining OpenSearch readers under the contract.** The new CI guard covers `core/retrieval.py` only. `citations/case_codal_suggestions.py` builds its own query and was checked by hand for this branch — its four `_source` fields (`document_id`, `title`, `citation_text`, `plain_text`) are all mapped on the keyword index, so it is correct **today** and simply unprotected against the next edit. `doctrines/service.py`'s `plain_text` read is from PostgreSQL sections, not OpenSearch, and is out of scope. Anything in `apps/api` that reads `_source` directly is unaudited.
