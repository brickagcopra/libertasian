...
> Last updated: 2026-08-19 (**new, top of the list: the Play production clock is still at day 0, and it is now the only thing between here and a public Android release.** Every App content declaration is complete, the store listing carries real screenshots, and **versionCode 11 sits on Closed testing – Alpha as a draft release**. Three things remain and none of them is code: (1) **Preview and confirm the release** on the alpha track — check the "What's new" box, EAS does not fill it; (2) **Publishing overview → Send app for review**, which is locked until (1) is done and which is deliberately left for a human because it is the irreversible outward-facing step; (3) **recruit testers — the list has 7, the rule needs 12 opted in *continuously* for 14 days, and the sane target is 15+ for headroom.** Google reviews the first closed release **before** the 14-day clock starts, so recruiting should run in parallel, not after. Also note: `eas.json` now carries `"releaseStatus": "draft"` — **flip it back to `completed` once the first review is approved**, or every future submit silently stops at draft. And `store/DATA_SAFETY.md` now documents two divergences between that file and the submitted console form (phone number, device IDs) that should be reconciled deliberately, not blindly.)
>
> Last updated: 2026-08-14 (**new, top of the list:** `feat/reranker-service` closes search-epic C4 — the retrieval stack is now structurally complete (BM25 + kNN + cross-encoder reranking). **Everything remaining is measurement and deployment, not construction.** The reranker needs `docker compose up -d` for a NEW service, and `abstention_score_threshold` moved 0.01 → 0.0004 on measured scores that must now be confirmed against prod traffic. See the section directly below.)
>
