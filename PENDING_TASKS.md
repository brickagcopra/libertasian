...
> Last updated: 2026-08-14 (**new, top of the list:** `fix/rag-embedding-client` makes retrieval genuinely hybrid for the first time — the kNN leg now runs. **What it does NOT do is fix ranking.** The right documents enter the candidate set, but under RRF the BM25 hit still ranks top-1 in 3 of 4 sampled queries. The reranker (search-epic C4) is what reorders them, and it is now the single highest-value piece of work left. See the section directly below.)
>
...
## Hybrid retrieval is live — ranking is the next problem (2026-08-14)

Full context: COMPLETED_TASKS.md under 2026-08-14 (two entries). `fix/knn-field-name` made the kNN query correct; `fix/rag-embedding-client` made it run. Neither reorders results.

- [ ] **Ship the reranker (search-epic C4). This is now the highest-value work left.** Measured on prod: passing a real embedding puts the right documents into the candidate set (Rosita Sy vs. People for "estafa", the actual Writ of Amparo petition for "writ of amparo", Lozano vs. People for "theft", De Castro vs. JBC for "stare decisis") — but under RRF the BM25 hit still ranks **top-1 in 3 of those 4**. RRF encodes rank, not relevance. Until a reranker scores relevance, the right passage is in context but not at the top of it.
- [ ] **`abstention_score_threshold` becomes tunable only after C4.** It is inert today: with no reranker, `rerank_score` is None and `check_abstention` compares a raw RRF score, which is structurally ~0.0164 at top-1 for any query returning anything. It starts meaning something the moment reranker scores exist — revisit then, not before.
- [ ] **Verify on prod that `degraded_legs` comes back EMPTY after deploy.** That is the acceptance evidence that the kNN leg is live: `degraded=false`, `degraded_legs=[]`. If it reports `knn:embedding_failed` the service is unreachable from rag-service (check the `ai` network and `RAG_EMBEDDING_SERVICE_URL`); if it reports `knn:not_configured` the env var did not reach the container — most likely the `RAG_` prefix.
- [ ] **Watch p50/p95 answer latency.** Every answer now makes an extra hop to embedding-service before retrieval, with a 5s timeout (`RAG_EMBEDDING_REQUEST_TIMEOUT`). CPU inference on `BAAI/bge-small-en-v1.5` for a short query should be tens of milliseconds, but it is unmeasured under real concurrency and it is on the critical path of every request.
- [ ] **Re-measure the authority boost and `compute_confidence` — together, and only now.** Both were deliberately left alone while retrieval was changing. `source_trust_level` (from #382) made the authority boost live for the first time, and the kNN leg changes which passages are scored at all. Measuring either before both landed would have confounded them. Sample real queries before/after rather than assuming improvement.
- [ ] **Bring the remaining OpenSearch readers under the contract guard.** Still true: the CI guard covers `core/retrieval.py` only. `citations/case_codal_suggestions.py` was checked by hand and is correct today, merely unprotected. Anything in `apps/api` reading `_source` directly is unaudited.
- [ ] **Widen the CI guard to the full rag-service suite once `test_routers.py` is repaired.** 40 pre-existing pytest-asyncio fixture errors are why it is scoped to one file.
